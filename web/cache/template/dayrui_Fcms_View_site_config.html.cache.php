...
<div id="dr_local" style="display: none">
    ...
    <div class="col-md-9">
        <label><select class="form-control" name="data[SITE_THEME]">
            ...
            <?php if (is_array($theme)) { $key_t=-1;$count_t=dr_count($theme);foreach ($theme as $t) { $key_t++; $is_first=$key_t==0 ? 1 : 0;$is_last=$count_t==$key_t+1 ? 1 : 0;?>
        ...
        <div class="portlet-title tabbable-line">
            <ul class="nav nav-tabs" style="float:left;">
                <li class="<?php if ($page==0) { ?>active<?php } ?>">
                    <a href="#tab_0" data-toggle="tab" onclick="$('#dr_page').val('0')"> <i class="fa fa-globe"></i> <?php echo dr_lang('网站设置'); ?> </a>
                </li>
                <li class="">
                    <a href="<?php echo dr_url("api/demo", ['name'=>'pc']); ?>" target="_blank"> <i class="fa fa-desktop"></i> <?php echo dr_lang('预览网站'); ?> </a>
                </li>
            </ul>
        </div>
                ...
                <div class="tab-pane <?php if ($page==0) { ?>active<?php } ?>" id="tab_0">
                    <div class="form-body">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-md-9">
                                <div class="mt-radio-inline">
                                    <label class="mt-radio mt-radio-outline"><input type="radio" name="data[SITE_INDEX_HTML]" value="1" <?php if ($data['SITE_INDEX_HTML']) { ?>checked<?php } ?> /> <?php echo dr_lang('开启'); ?> <span></span></label>
                                    <label class="mt-radio mt-radio-outline"><input type="radio" name="data[SITE_INDEX_HTML]" value="0" <?php if (empty($data['SITE_INDEX_HTML'])) { ?>checked<?php } ?> /> <?php echo dr_lang('关闭'); ?> <span></span></label>
                                </div>
                                <span class="help-block"><?php echo dr_lang('自动生成首页静态index.html文件'); ?></span>
                            </div>
                        </div>
                        <div class="form-group">
                            <label class="col-md-2 control-label"><?php echo dr_lang('网站状态'); ?></label>
                            <div class="col-md-9">
                                <div class="mt-radio-inline">
                                    <label class="mt-radio mt-radio-outline"><input type="radio" onclick="$('.dr_close_msg').hide()" name="data[SITE_CLOSE]" value="0" <?php if (empty($data['SITE_CLOSE'])) { ?>checked<?php } ?> /> <?php echo dr_lang('开启'); ?> <span></span></label>
                                    <label class="mt-radio mt-radio-outline"><input type="radio" onclick="$('.dr_close_msg').show()" name="data[SITE_CLOSE]" value="1" <?php if ($data['SITE_CLOSE']) { ?>checked<?php } ?> /> <?php echo dr_lang('关闭'); ?> <span></span></label>
                                </div>
                                <span class="help-block"><?php echo dr_lang('当关闭网站时，除管理员之外的用户将无法访问（静态页面除外）'); ?></span>
                            </div>
                        </div>
                        <div class="form-group dr_close_msg">
                            <label class="col-md-2 control-label"><?php echo dr_lang('关闭理由'); ?></label>
                            <div class="col-md-9">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-md-9">
                                <label><input class="form-control input-large" type="text" name="data[SITE_NAME]" id="dr_name" value="<?php echo htmlspecialchars($data['SITE_NAME']); ?>"></label>
                            </div>
                        </div>

                        <div class="form-group">
                            <label class="col-md-2 control-label"><?php echo dr_lang('网站语言'); ?></label>
                            <div class="col-md-9">
                                <label><select class="form-control" name="data[SITE_LANGUAGE]">
                                    <option value="zh-cn"> -- </option>
                                    <?php if (is_array($lang)) { $key_t=-1;$count_t=dr_count($lang);foreach ($lang as $t) { $key_t++; $is_first=$key_t==0 ? 1 : 0;$is_last=$count_t==$key_t+1 ? 1 : 0;?>
                                    <option<?php if ($t==$data['SITE_LANGUAGE']) { ?> selected=""<?php } ?> value="<?php echo $t; ?>"><?php echo $t; ?></option>
                                    <?php } } ?>
                                </select></label>
                                <span class="help-block"><?php echo dr_lang('为站点设置不同的语言包'); ?></span>
                            </div>
                        </div>
                        <div class="form-group">
                            <label class="col-md-2 control-label"><?php echo dr_lang('风格模式'); ?></label>
                            <div class="col-md-9">
                                <div class="mt-radio-inline">
                                    <label class="mt-radio mt-radio-outline"><input type="radio" onclick="dr_theme(1)" name="theme" value="1" <?php if ($is_theme) { ?>checked<?php } ?> /> <?php echo dr_lang('远程地址'); ?> <span></span></label>
                                    <label class="mt-radio mt-radio-outline"><input type="radio" onclick="dr_theme(0)" name="theme" value="0" <?php if (!$is_theme) { ?>checked<?php } ?> /> <?php echo dr_lang('本站资源'); ?> <span></span></label>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="form-group">
                            <label class="col-md-2 control-label"><?php echo dr_lang('模板目录'); ?></label>
                            <div class="col-md-9">
                                <label><select class="form-control" name="data[SITE_TEMPLATE]">
                                    <option value="default"> -- </option>
                                    <?php if (is_array($template_path)) { $key_t=-1;$count_t=dr_count($template_path);foreach ($template_path as $t) { $key_t++; $is_first=$key_t==0 ? 1 : 0;$is_last=$count_t==$key_t+1 ? 1 : 0;?>
                                    <option<?php if ($t==$data['SITE_TEMPLATE']) { ?> selected=""<?php } ?> value="<?php echo $t; ?>"><?php echo $t; ?></option>
                                    <?php } } ?>
                                </select></label>
                                <span class="help-block"><?php echo dr_lang('存放网站模板html文件的目录'); ?></span>
                            </div>
                        </div>
                        <div class="form-group">
                            <label class="col-md-2 control-label"><?php echo dr_lang('网站时区'); ?></label>
                            <div class="col-md-9">
                                ...
                                <span class="help-block"><?php echo dr_lang('例如中国地区选择“GMT +08:00”表示东八区'); ?></span>
                            </div>
                        </div>
                        <div class="form-group">
                            <label class="col-md-2 control-label"><?php echo dr_lang('时间格式'); ?></label>
                            <div class="col-md-9">
                                <label><input class="form-control" type="text" name="data[SITE_TIME_FORMAT]" value="<?php echo htmlspecialchars($data['SITE_TIME_FORMAT']); ?>"></label>
                                <span class="help-block"><?php echo dr_lang('网站时间显示格式与date函数一致，默认Y-m-d H:i:s'); ?></span>
                            </div>
                        </div>
