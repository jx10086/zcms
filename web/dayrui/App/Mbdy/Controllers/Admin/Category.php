...
class Category extends \Phpcmf\App
{
    ...
    public function cms() {
        ...
        $mdata = \Phpcmf\Service::L('cache')->get('module-'.SITE_ID.'-'.($mid ? $mid : 'share'));
        if (!$mdata['category'][$id]) {
            $this->_admin_msg(0, dr_lang('栏目['.$id.']缓存不存在，尝试更新缓存'));
        }
    ...
    public function tag() {
        ...
        $mdata['category_field'] = dr_array22array($this->mrfield, $mdata['category_field']);
